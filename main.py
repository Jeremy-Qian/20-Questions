...
class TwentyQuestionsAI:
    ...
    def make_guess(self):
        ...
        if response == 'yes':
            ...
            add_to_db = input("Would you like to add the correct person to my database? (yes/no): ").strip().lower()
            if add_to_db == 'yes':
                ...
                if correct_person not in self.people_data:
                    ...
                    for question in self.questions:
                        if (question not in self.people_data[correct_person] or 
                            self.people_data[correct_person][question] == 'unknown'):
                            print(f"\nQuestion about {correct_person}: {question}")
                            while True:
                                answer = input("What's the correct answer (yes/no/unknown): ").strip().lower()
